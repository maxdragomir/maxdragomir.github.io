...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var is_opera = !!window.opera || navigator.userAgent.indexOf(" OPR/") >= 0,
      ...
      direction = false; // Start

  setTimeout(function () {
    wrapper.classList.add("".concat(lotteryName, "-loaded"));
  }, 500); // Hash

  var hash = location.search.replace(/\?/, ""),
      rtlSlider = document.querySelector(".".concat(lotteryName, "-section--slider"));

  if (hash === "righttoleft") {
    ltr.parentNode.removeChild(ltr);
    rtlSlider.classList.add("".concat(lotteryName, "-section--rtl"));
    direction = true;
  } else {
  ...
  function mobAdaptive() {
    var second = document.querySelector(".".concat(lotteryName, "-block__row--second")),
        third = document.querySelector(".".concat(lotteryName, "-block__row--third")),
        thirdAll = document.querySelectorAll(".".concat(lotteryName, "-block__row--third > div")),
        thirdItem = thirdAll[0],
        moveItem = document.querySelector(".".concat(lotteryName, "-block__item--move"));

    if (window.innerWidth <= 1000) {
      second.appendChild(moveItem);
    } else {
